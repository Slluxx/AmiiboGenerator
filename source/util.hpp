...
public:
    ...
    static void delete_autogenerated_amiibos()
    {

        // filesystem loop through amiibo folder (series)
        for (const auto &entry : std::filesystem::directory_iterator("sdmc:/emuiibo/amiibo/"))
        {
            // loop through its subfolder (amiibo)
            for (const auto &subentry : std::filesystem::directory_iterator(entry.path()))
            {
                if (check_file_exist(subentry.path().string() + "/autogenerated.flag"))
                {
                    delete_folder_contents(subentry.path().string());
                }
            }
